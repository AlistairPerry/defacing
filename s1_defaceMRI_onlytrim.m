function s1_defaceMRI_onlytrim(T1_DIR)

%Adapted from X
%Only performs trimming for computational resources
%Requires fieldtrip to be in search path
...
% Gets the list of input MRIs in provided DIR
...
for findex = 1: numel ( files )
    ...
    %AP 04/11/22 - Indent out defacing commands and output
    ...
    fprintf ( 1, '  Saving the trimmed MRI data.\n' )
    
    % Saves the trimmed data in NIfTI format.
    out_fname = [basename '_' 'trim'];
    
    %     ft_write_mri ( sprintf ( '%s%s.nii', config.path.def, basename ), mri_def.anatomy, 'transform', mri.transform, 'dataformat', 'nifti' )
    ft_write_mri ( sprintf ( '%s%s.nii', T1_DIR, out_fname ), mri_trim.anatomy, 'transform', mri.transform, 'dataformat', 'nifti' )
